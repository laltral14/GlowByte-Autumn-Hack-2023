...
class HolidaysTransformer(BaseEstimator, TransformerMixin):
    ...
    def transform(self, data):
        ...
        data['holiday'] = data['ds'].dt.date.isin(df_holidays_train['ds'].dt.date).apply(lambda x: 1 if x else 0)
...
class CovidTransformer(BaseEstimator, TransformerMixin):
    ...
    def transform(self, data):
        ...
        data['covid'] = data['ds'].dt.date.isin(df_covid_train['ds'].dt.date).apply(lambda x: 1 if x else 0)

        return data
    

class WorkingHoursTransformer(BaseEstimator, TransformerMixin):
    def fit(self, X, y=None):
        return self

    def transform(self, data):
        
        # Create a column 'working_hours' and set 1 for hours between 8 and 20, and 0 for other hours
        data['working_hours'] = ((data['hour'] >= 8) & (data['hour'] <= 20)).astype(int)
